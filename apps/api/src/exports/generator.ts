...
export function envExample(plan: ExportPlan): string {
  const lines = [
    ...
    '',
    '# ── SEGURIDAD (leer esto) ──',
    '# El bot queda con una URL pública. SIN RUNTIME_API_KEY, cualquiera que la conozca',
    '# puede leer todas las conversaciones, mandar mensajes haciéndose pasar por el equipo,',
    '# y disparar una campaña a la base entera. Generala con: openssl rand -hex 32',
    'RUNTIME_API_KEY=',
    '#',
    '# App Secret de Meta: verifica la firma de cada webhook de WhatsApp. Sin esto',
    '# cualquiera postea un mensaje inventado y el bot lo procesa como real.',
    '# Está en developers.facebook.com → tu app → Configuración → Básica.',
    'WHATSAPP_APP_SECRET=',
    '#',
    '# Token del webhook de mail (solo si un proveedor postea a /webhooks/email;',
    '# no hace falta si el mail entra por IMAP).',
    '# EMAIL_WEBHOOK_TOKEN=',
    '#',
    '# Freno por IP (por minuto). Evita que un bucle dispare miles de llamadas a la IA.',
    '# RATE_LIMIT_MAX=120',
    '# RATE_LIMIT_WINDOW_MS=60000',
    '',
    '# ── Mail entrante por IMAP (opcional) ──',
    '# DonWeb/Ferozo: el host sale del panel → Correos → "Datos de configuración".',
    '# IMAP_HOST=usuario.ferozo.com',
    '# IMAP_USER=coordinacion@tudominio.com',
    '# IMAP_PASSWORD=',
    '# IMAP_PORT=993          # 993 = SSL (default). 143 = sin SSL.',
    '# IMAP_MAILBOX=INBOX',
    '# IMAP_POLL_SECONDS=60   # mínimo 15',
    '# IMAP_FAILED_MAILBOX=INBOX.Fallidos   # los mails que fallan van acá, no se pierden',
    '',
    '# ── Agrupado de mensajes seguidos ──',
    '# La gente manda "hola", "queria consultar", "por las categorias" en 2 segundos.',
    '# Se esperan estos ms desde el último mensaje y se contestan todos juntos. 0 lo apaga.',
    '# INBOUND_DEBOUNCE_MS=6000',
    '',
